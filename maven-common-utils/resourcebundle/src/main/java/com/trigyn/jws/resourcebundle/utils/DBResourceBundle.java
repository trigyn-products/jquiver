...
public class DBResourceBundle extends AbstractMessageSource {
	...
	protected MessageFormat resolveCode(String a_key, Locale a_locale) {
		...
		message = iResourceBundleRepository.findByKeyAndLanguageCode(a_key,a_locale.toString(),DEFAULT_LOCALE_CODE.toString(), Constant.RecordStatus.INSERTED.getStatus());
		if(message != null) {
