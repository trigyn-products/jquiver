...
public final class QueryStore {
        ...
        protected static final String HQL_QUERY_FIND_DASHBOARD_BY_CONTEXT_ID = " FROM Dashboard AS d INNER JOIN d.dashboardRoles AS dr where dr.roleId IN ( :userRoles ) AND d.contextId = :contextId AND d.createdBy IN ( :adminStr, :userId ) AND d.isDeleted = 0 ORDER BY d.lastUpdatedDate DESC";
        ...
        public static final String JPA_QUERY_TO_GET_ALL_CONTEXT = "SELECT new app.trigyn.common.dashboard.vo.ContextMasterVO "
        		+ " (cm.contextId AS contextId, cm.contextDescription AS contextDescription) "
        		+ " FROM ContextMaster AS cm ";

        public static final String JPA_QUERY_TO_GET_DASHLET_BY_CONTEXT_ID = "SELECT new  app.trigyn.common.dashboard.vo.DashboardDashletVO "
        		+ " (da.id.dashboardId AS dashboardId, dl.dashletId AS dashletId, dl.dashletName AS dashletName, dl.contextId AS contextId ) "
        		+ " FROM Dashlet AS dl "
        		+ " LEFT OUTER JOIN dl.contextMaster AS cm "
        		+ " LEFT OUTER JOIN dl.dashboardAssociation AS da ON da.id.dashboardId = :dashboardId "
        		+ " WHERE dl.contextId = :contextId ";
        		
                        
        public static final String JPA_QUERY_TO_GET_DAHSLET_DETAILS_BY_ID = "SELECT new app.trigyn.common.dashboard.vo.DashletVO "
        		...
        		+ " , dl.width AS width, dl.height AS height, dl.contextId AS contextId "
        		+ " , dl.showHeader AS showHeader, dl.isActive AS isActive) "
        		+ " FROM Dashlet AS dl WHERE dl.dashletId = :dashletId ";
        
        public static final String JPQ_QUERY_TO_GET_DAHSLET_PROPERTIES_BY_ID = "SELECT new app.trigyn.common.dashboard.vo.DashletPropertyVO "
        		...
        		+ " , dp.toDisplay AS toDisplay, dp.sequence AS sequence ) "
        		+ " FROM DashletProperties AS dp "
        		+ " WHERE dp.dashletId = :dashletId AND dp.isDeleted = :isDeleted ORDER BY dp.id.sequence ASC" ;
        
        
        public static final String JPQ_QUERY_TO_GET_DASHLET_ROLES_ASSOCIATION_BY_ID = "SELECT new app.trigyn.common.dashboard.vo.DashletRoleAssociationVO "
        		+ " (dra.id.roleId AS roleId, dra.id.dashletId AS dashletId) "
        		+ " FROM DashletRoleAssociation AS dra "
        ...
        public static final String JPA_QUERY_TO_GET_LOOKUP_DETAILS_BY_CATEOGRY_NAME = "SELECT new app.trigyn.common.dashboard.vo.DashboardLookupCategoryVO "
