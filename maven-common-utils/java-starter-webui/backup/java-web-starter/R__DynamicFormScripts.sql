	...
	$(function () {
		...
		sessionStorage.setItem("dynamic-form-manage-details", formDataJson);
			...
			let colM = [
				...
				{ title: "Action", width: 50, maxWidth: 145, dataIndx: "action", align: "left", halign: "left", render: editDynamicFormFormatter, sortable: false}
			];
			let dataModel = {
		        ...
		        <#if loggedInUserRoleList?? && loggedInUserRoleList?size gt 0>
		        	<#list loggedInUserRoleList as loggedInUserRole>
		            	<#if (loggedInUserRole == "ADMIN")>   
		        			actionElement += ''<span onclick=\\''openDeletConfirmation(\"divDynamicFormMasterGrid",\"''+dynamicFormId+''\",\"30a0ff61-0ecf-11eb-94b2-f48e38ab9348\")\\'' class= "grid_action_icons" title="Delete"><i class="fa fa-trash "></i></span>''.toString();
		        			<#break>
		        		</#if>
		        	</#list>
		        </#if>
...
<div class="container">
    ...
    <form id="dynamicform" method="post" >
		...
		<div class="row">
			...
			<div class="col-6">
				<div class="col-inner-form full-form-fields">
	        		<label for="flammableState" style="white-space:nowrap">Query Type</label>
	        		<select id="selectQueryType" name="selectQueryType" class="form-control" onchange="updateSelectDataSource(this)">
	        			<option id="selectQueryType" value="1" selected>Select Query</option>
	        			<option id="jsQueryType" value="2">Javascript</option>
	        		</select>
           			</div>
			</div>
...
<script>
	...
	const addEdit = new AddEditDynamicForm();
  	...
  	$(function () {
  		 ...
  		 $("#primaryKey").val(''${(dynamicForm?api.getFormId())!''''}'');
	    ...
	    hideShowActionButtons();
	     ...
	     <#if !(dynamicForm.formId)?? && !(dynamicForm.formId)?has_content>
			...
			$("#formId").val(generatedFormId);
			$("#primaryKey").val(generatedFormId);
		<#else>
			getAllDatasource(1);
