...
if(obj != null && obj["encryptionAlgoName"] == "NA") {
  ...
  function validateTestEncDec(secretKey) {
       if(autocomplete.getSelectedObject()["encryptionAlgoName"] == "NA") {
            $("#errorMessage").html("No algorithm is selected");
			$("#errorMessage").show();
            return false;   
      } else if($("#inputText").val() == null || $("#inputText").val() == undefined || $("#inputText").val() == "") {
            $("#errorMessage").html("Test input is empty.");
			$("#errorMessage").show();
            return false;
      } else if(secretKey == null || secretKey == undefined || secretKey == "") {
            $("#errorMessage").html("Client secret key is required to test");
			$("#errorMessage").show();
            return false;
      } else if(autocomplete.getSelectedObject()["encryptionAlgoName"] == "DES" && secretKey.length <= 25) {
          $("#errorMessage").html("Client secret key should be more than 25 characters for DES");
			$("#errorMessage").show();
            return false;
      }
      return true;
