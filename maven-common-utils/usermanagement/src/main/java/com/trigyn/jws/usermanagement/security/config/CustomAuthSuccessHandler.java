	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if (user.getForcePasswordChange() == 1) {
			...
			redirectUrl = new StringBuilder().append(servletContext.getContextPath())
					.append("/cf/changePassword?token=").append(user.getUserId()).toString();
		} else {
			try {
				if (applicationSecurityDetails.getAuthenticationType() != null) {
					...
					if (StringUtils.isNotBlank(authenticationType.getAuthenticationProperties())) {
						...
						jsonObjectVerificationType = userConfigService.getJsonObjectFromPropertyValue(
								jsonObjectVerificationType, jsonArray, verificationStepPropertyName);
						if (jsonObjectVerificationType != null
								&& jsonObjectVerificationType.getString("value").equalsIgnoreCase("true")) {
							if (jsonObjectVerificationType.get("selectedValue") != null && Integer
									.parseInt(jsonObjectVerificationType.get("selectedValue").toString()) != 2) {

								jsonObjectCaptcha = userConfigService.getJsonObjectFromPropertyValue(
										jsonObjectVerificationType, jsonArray, "enablePasswordExpiry");
								Integer		expiryDays	= Integer
										.parseInt(jsonObjectCaptcha.get("selectedValue").toString());
