...
import app.trigyn.core.dynarest.dao.JwsDynamicRestDAORepository;
import app.trigyn.core.dynarest.dao.JwsDynarestDAO;
...
public class DynarestCrudService {
	...
	public void downloadDynamicRestCode() throws Exception {
		...
		downloadFormsToLocal(dynamicRestDetails);
		
	}

	public void downloadFormsToLocal(List<JwsDynamicRestDetail> dynamicRestDetails) throws Exception, IOException {
        	...
        	if(dynamicRestDetail ==  null) {
				...
				if(filesPresent >= 2) {
					...
					if(!serviceLogicFile.exists() ) {
						...
						dynamicRestDAORepository.saveAll(dynamicRestDaoDetails);
