...
public class ImportExportCrudDAO extends DBConnection {
	...
	public List<Map<String, Object>> getAllCustomEntity() {
		...
		public List<Map<String, Object>> getCustomEntityCount() {
			String querySQL = 
					"SELECT count(*) as count, 'Grid' as enityType"
					+ " FROM grid_details gd WHERE gd.grid_type_id = 1"
					+ " UNION "
					+ " SELECT count(*) as count, 'Templates' as enityType"
					+ " FROM template_master tm WHERE tm.template_type_id = 1"
					+ " UNION "
					+ " SELECT count(*) as count, 'ResourceBundle' as enityType"
					+ " FROM resource_bundle rb WHERE rb.resource_key NOT LIKE 'jws.%'"
					+ " UNION "
					+ " SELECT count(*) as count, 'Autocomplete' as enityType"
					+ " FROM autocomplete_details au WHERE au.ac_type_id = 1"
					+ " UNION "
					+ " SELECT count(*) as count, 'Notification' as enityType"
					+ " FROM generic_user_notification gun"
					+ " UNION "
					+ " SELECT count(*) as count, 'Dashboard' as enityType"
					+ " FROM dashboard db WHERE db.dashboard_type = 1"
					+ " UNION "
					+ " SELECT count(*) as count, 'Dashlets' as enityType"
					+ " FROM dashlet dl WHERE dl.dashlet_type_id = 1"
					+ " UNION "
					+ " SELECT count(*) as count, 'DynamicForm' as enityType"
					+ " FROM dynamic_form df WHERE df.form_type_id = 1"
					+ " UNION "
					+ " SELECT count(*) as count, 'FileManager' as enityType"
					+ " FROM file_upload_config fu"
					+ " UNION "
					+ " SELECT count(*) as count, 'DynaRest' as enityType"
					+ " FROM jws_dynamic_rest_details dr WHERE dr.jws_dynamic_rest_type_id = 1 "
					+ " UNION "
					+ " SELECT 0 as count, 'Permission' as enityType"
					+ " FROM jws_role jr"
					+ " UNION "
					+ " SELECT 0 as count, 'SiteLayout' as enityType"
					+ " FROM module_listing ml"
					+ " UNION "
					+ " SELECT 0 as count, 'ApplicationConfiguration' as enityType"
					+ " FROM jws_property_master"
					+ " UNION "
					+ " SELECT 0 as count, 'ManageUsers' as enityType"
					+ " FROM jws_user"
					+ " UNION "
					+ " SELECT 0 as count, 'ManageRoles' as enityType"
					+ " FROM jws_role"
					+ "";
		...
		public List<Map<String, Object>> getAllEntityCount() {
			String querySQL = 
					"SELECT count(*) as totalCount, 'Grid' as enityType"
					+ " FROM grid_details"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'Templates' as enityType"
					+ " FROM template_master"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'ResourceBundle' as enityType"
					+ " FROM resource_bundle"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'Autocomplete' as enityType"
					+ " FROM autocomplete_details"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'Notification' as enityType"
					+ " FROM generic_user_notification gun"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'Dashboard' as enityType"
					+ " FROM dashboard"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'Dashlets' as enityType"
					+ " FROM dashlet"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'DynamicForm' as enityType"
					+ " FROM dynamic_form"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'FileManager' as enityType"
					+ " FROM file_upload_config"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'DynaRest' as enityType"
					+ " FROM jws_dynamic_rest_details"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'Permission' as enityType"
					+ " FROM (SELECT jera.entity_role_id AS entityRoleId FROM  jws_role jr RIGHT OUTER JOIN jws_entity_role_association jera ON jera.role_id = jr.role_id INNER JOIN jws_master_modules jmm"
					+ " ON jmm.module_id = jera.module_id WHERE jr.is_active=1  AND jera.module_type_id=0 GROUP BY jera.entity_id) tableName"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'SiteLayout' as enityType"
					+ " FROM module_listing ml"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'ApplicationConfiguration' as enityType"
					+ " FROM jws_property_master"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'ManageUsers' as enityType"
					+ " FROM jws_user"
					+ " UNION "
					+ " SELECT count(*) as totalCount, 'ManageRoles' as enityType"
					+ " FROM jws_role"
					+ "";
