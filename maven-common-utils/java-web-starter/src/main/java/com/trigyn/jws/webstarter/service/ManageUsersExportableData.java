...
@Component
public class ManageUsersExportableData implements GenerateModuleMasterQueries {

	private final static Logger logger = LoggerFactory.getLogger(ManageUsersExportableData.class);
	...
	public List<Object> generateDynamicModuleQuery(List<String> systemConfigIncludeList,
		...
		if (autoExport) {
			StringBuilder querySQL = new StringBuilder(
					CrudQueryStore.HQL_QUERY_TO_FETCH_MANAGE_USERS_DATA_FOR_AUTO_EXPORT);
			if (modifiedAfter != null) {
				...
				querySQL.append("ju.last_password_updated_date  >=:modifiedAfter ");
				// params.addValue("modifiedAfter", modifiedAfter);
			}
			BeanPropertyRowMapper<JwsUser> mapper = new BeanPropertyRowMapper<JwsUser>(JwsUser.class);
			exportableList = importExportCrudDAO.getAllAutoExportableData(
					querySQL.toString(), modifiedAfter,null ,name,mapper);
