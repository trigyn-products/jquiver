...
public class JwsSchedulerJob implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		String		schedulerId				= (String) context.getJobDetail().getJobDataMap().get("schedulerId");
		String		baseURL					= (String) context.getJobDetail().getJobDataMap().get("baseURL");
		String		schedulerUrlProperty	= (String) context.getJobDetail().getJobDataMap()
				.get("schedulerUrlProperty");
		...
		try {
			...
			execute(schedulerId, baseURL, schedulerUrlProperty, contextPath, userName);
		} catch (Exception exec) {
			logger.error("Error occured while executing Scheduler : " + "Scheduler ID : "
					+ schedulerId, exec);
		}
	}

	public void execute(String schedulerId, String baseURL,
			String schedulerUrlProperty, String contextPath, String userName) throws AddressException, Exception {
		Date	requestTime	= null;
		Gson	g			= new Gson();
		JqScheduler jqScheduler = jqschedulerRepository.findById(schedulerId).orElse(null);
		if(jqScheduler!=null) {
			try {
				...
				Builder builder = WebClient.builder().exchangeStrategies(ExchangeStrategies.builder()
						.codecs(clientCodecConfigurer -> clientCodecConfigurer.defaultCodecs().maxInMemorySize(10000000))
						.build()).baseUrl(restApiUrl).defaultHeader(HttpHeaders.CONTENT_TYPE, "application/json")
				...
				Mono<ResponseEntity<String>>	responseContent	= webClient.method(HttpMethod.resolve(requestType))
						...
						.onStatus(HttpStatus::is4xxClientError, response -> {
																			return response
																					...
																					.flatMap(error -> {
																																		return Mono
																																				.error(new CustomRuntimeException(
																																						error));
																																	});
																		})
