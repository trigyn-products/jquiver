...
@Component
public class AutocompleteExportableData implements GenerateModuleMasterQueries {

	private final static Logger logger = LoggerFactory.getLogger(AutocompleteExportableData.class);
	...
	public List<Object> generateDynamicModuleQuery(List<String> systemConfigIncludeList,
		...
		if (autoExport) {
			StringBuilder querySQL = new StringBuilder(
					CrudQueryStore.HQL_QUERY_TO_FETCH_AUTOCOMPLETE_DATA_FOR_AUTO_EXPORT);
			if (modifiedAfter != null) {
				...
				querySQL.append("COALESCE(au.last_updated_ts , au.created_date)  >=:modifiedAfter ");
		    ...
		    if (entityType != null) {
				...
				querySQL.append(" au.ac_type_id = :entityType ");
			}
			BeanPropertyRowMapper<Autocomplete> mapper = new BeanPropertyRowMapper<Autocomplete>(Autocomplete.class);
