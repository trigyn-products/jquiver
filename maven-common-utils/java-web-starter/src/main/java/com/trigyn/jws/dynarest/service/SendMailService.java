...
public class SendMailService {
	...
	public CompletableFuture<Boolean> sendTestMail(Email mail) throws Exception {
		...
		String			mailBody			= "";
		boolean			isAllowCustomSender	= true;
		if (mailMap.get(IS_ALLOW_CUSTOM_SENDER) != null) {
			isAllowCustomSender = Boolean.parseBoolean(mailMap.get(IS_ALLOW_CUSTOM_SENDER).toString());
		}
		String				fromMailId				= (isAllowCustomSender && mail.getMailFrom() != null
				&& mail.getMailFrom()[0].getAddress() != null) ? mail.getMailFrom()[0].getAddress()
						: (String) mailMap.get("mailFrom");

		/** Fixed Bug #212, changed the boolean value from true to false. */
		String				fromMailName			= (isAllowCustomSender
				&& StringUtils.isBlank(mail.getMailFromName()) == false) ? mail.getMailFromName()
		...
		String				mailFooterString	= (mailMap.get("mailFooter") != null
				&& !mailMap.get("mailFooter").equals("") ? (String) mailMap.get("mailFooter") : mail.getMailFooter());
		...
		try {
				...
				if (isAllowed > 0) {
					...
					if (fileInfo != null) {
						...
						try (FileOutputStream fos = new FileOutputStream(attachedFile)) {
							...
							logger.error("Error occurred while accessing file : fileUploadId : " + fileUploadId,
									exception);
						}
						BodyPart	atachmentBodyPart	= new MimeBodyPart();
			...
			if (mail != null && mail.getMailFooter() != null) {
				...
				if (mailFooter != null) {
					mailFooter = templatingUtils.processMultipleTemplateContents(mailFooter, "footer",
	...
	public CompletableFuture<Boolean> sendTestMail(Email mail, String jsonString) throws Exception {
		...
		boolean			isAllowCustomSender	= true;
		if (mailMap.get(IS_ALLOW_CUSTOM_SENDER) != null) {
			isAllowCustomSender = Boolean.parseBoolean(mailMap.get(IS_ALLOW_CUSTOM_SENDER));
		}
		String	fromMailId				= (isAllowCustomSender && mail.getMailFrom() != null
				&& mail.getMailFrom()[0].getAddress() != null) ? mail.getMailFrom()[0].getAddress()
						: (String) mailMap.get("mailFrom");

		String	fromMailName			= (isAllowCustomSender && StringUtils.isBlank(mail.getMailFromName()) == false)
		...
		/** Added for ReplyTo tag */
		String			replyTo					= (mail.getReplyToaddress() != null
				&& mail.getReplyToaddress()[0].getAddress() != null) ? mail.getReplyToaddress()[0].getAddress()
						: (String) mailMap.get("replyToDifferentMailId");
		/** Ends Here */
	...
	public boolean sendMail(EmailSchedulerRequestVo emailVo) throws Exception {
		...
		Authenticator		authenticator	= null;
		Session				session			= null;
		Properties			prop			= JobUtil.getSmtpDetails(mailMap);

		Boolean				smtpAuth		= mailMap.get("isAuthenticated") != null
				? (Boolean) mailMap.get("isAuthenticated")
				: null;
		boolean				isSent			= true;
		if (smtpAuth != null && Boolean.TRUE.equals(smtpAuth)) {
			prop.put("mail.smtp.auth", smtpAuth);
		...
		BodyPart			messageBodyPart			= new MimeBodyPart();
		String				mailBody				= "";
		Email				mail					= emailVo.getEmail();
		String				fromMailId				= (mail.getMailFrom() != null
				&& mail.getMailFrom()[0].getAddress() != null) ? mail.getMailFrom()[0].getAddress()
						: (String) mailMap.get("mailFrom");

		/** Fixed Bug #212, changed the boolean value from true to false. */
		String				fromMailName			= StringUtils.isBlank(mail.getMailFromName()) == false
				? mail.getMailFromName()
				: (String) mailMap.get("mailFromName");
		/** Ends Here */
		...
		try {
			...
			isSent = false;
			logger.error("Error ocurred.", a_excep.getCause());
			if (a_excep.getCause() instanceof CannotGetJdbcConnectionException
					|| a_excep.getCause() instanceof SQLNestedException
					|| a_excep.getCause() instanceof SQLNonTransientConnectionException
					|| a_excep.getCause() instanceof ConnectException) {
				logger.error("Error ocurred.", a_excep);
				if (mailBody != "") {
					messageBodyPart.setContent(mailBody, "text/html; charset=utf-8");
				} else if (mail.getBody() != null && mail.getBody() != "") {
				...
				try {
					...
					if (mailToList.length == 0) {
						...
						String mailStr = "";
						for (int receipientCounter = 0; receipientCounter < mailToList.length; receipientCounter++) {
							mailStr += mailToList[receipientCounter].getAddress();
						}
						mailHistory.setMailSentTo(mailStr);
					}
					...
					jobDataMap.put("dynamicRestUrl", emailVo.getDynamicRestUrl());
	...
	public void saveMailHistory(String emailXMLContent, String groupId) throws JAXBException {

		EmailListXMLVO emailListObj = JobUtil.marshalEmailListXML(emailXMLContent);
		if (emailListObj != null && emailListObj.getEmailXMLVO() != null) {
			for (EmailXMLVO emailObj : emailListObj.getEmailXMLVO()) {
				MailSchedule bulkMailRequest = new MailSchedule();
				bulkMailRequest.setEmailXml(JobUtil.emailXmlmarshaling(emailObj));
				bulkMailRequest.setMailScheduleGroupId(groupId);
				mailHistoryRepository.save(bulkMailRequest);
				// sendMailDao.saveMails(bulkMailRequest);
			}
		}
	}
}
