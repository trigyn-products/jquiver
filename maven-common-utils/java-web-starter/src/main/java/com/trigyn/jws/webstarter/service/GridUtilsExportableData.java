...
@Component
public class GridUtilsExportableData implements GenerateModuleMasterQueries {

	private final static Logger logger = LoggerFactory.getLogger(GridUtilsExportableData.class);
	...
	public List<Object> generateDynamicModuleQuery(List<String> systemConfigIncludeList, List<String> customConfigExcludeList,
		...
		if (autoExport) {
			StringBuilder querySQL = new StringBuilder(CrudQueryStore.HQL_QUERY_TO_FETCH_GRID_DATA_FOR_AUTO_EXPORT);
			if (modifiedAfter != null) {
				...
				querySQL.append("COALESCE(gd.last_updated_ts , gd.created_date)  >=:modifiedAfter ");
			}
			...
			if (entityType != null) {
				...
				querySQL.append(" gd.grid_type_id = :entityType ");
			}
			BeanPropertyRowMapper<GridDetails> mapper = new BeanPropertyRowMapper<GridDetails>(GridDetails.class);
