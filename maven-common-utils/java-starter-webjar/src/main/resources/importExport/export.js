		...
		if (exportObj == null || (exportObj != null && exportObj.getColM() == null)) {
			...
			if (moduleType == "Grid") {
				...
				colM = [
					...
					{
						title: "Template Name", width: 130, align: "center", sortable: true, dataIndx: "templateName", align: "left", halign: "center", render: formattemplateName,
						filter: { type: "textbox", condition: "contain", listeners: ["change"] }
					},
				...
				colM = [
					...
					{
						title: "Method Name", width: 100, align: "center", dataIndx: "methodName", align: "left", halign: "center", render: formatmethodName,
						filter: { type: "textbox", condition: "contain", listeners: ["change"] }
					},
				...
				colM = [
					...
					{
						title: "Module Name", width: 100, align: "center", dataIndx: "moduleName", align: "left", halign: "center", render: formatmoduleName,
						filter: { type: "textbox", condition: "contain", listeners: ["change"] }
					},
					{
				...
				colM = [
					...
					{
						title: "Property Value", width: 130, dataIndx: "propertyValue", align: "left", align: "left", halign: "center", render: formatpropertyValue,
						filter: { type: "textbox", condition: "contain", listeners: ["change"] }
					},
				...
				colM = [
					...
					{
						title: "First Name", width: 130, align: "center", dataIndx: "firstName", align: "left", halign: "center", render: formatfirstName,
						filter: { type: "textbox", condition: "contain", listeners: ["change"] }
					},
					{
						title: "Last Name", width: 130, align: "center", dataIndx: "lastName", align: "left", halign: "center", render: formatlastName,
						filter: { type: "textbox", condition: "contain", listeners: ["change"] }
					},
					{
				...
				colM = [
					{
						title: "<button type='button' class='select_btn_new'  onclick='selectAll(\"" + moduleType + "\",true)'><i class='fa fa-check-square selectallcls' ></i></button>"
							+ "<button type='button' class='select_btn_new'  onclick='selectAll(\"" + moduleType + "\",false)'><i class='fa fa-window-close deslectcls' ></i></button> ", width: 90, maxWidth: 90, align: "center", sortable: false, render: updateScriptLibFormatter, dataIndx: ""
					},
					{ title: "", hidden: true, sortable: false, dataIndx: "script_lib_id" },
					{
						title: "Library Name", width: 130, align: "center", sortable: true, dataIndx: "library_name", align: "left", halign: "center",
						filter: { type: "textbox", condition: "contain", listeners: ["change"] }
					},
					{ title: "Updated By", width: 160, align: "center", sortable: true, dataIndx: "updated_by", align: "left", halign: "center" },
					{ title: "Updated Date", width: 200, align: "center", sortable: true, dataIndx: "updated_date", align: "left", halign: "center", render: scriptLibraryDateRenderer }
				];
				sortIndex = 2;
				isSelectTypeApplicable = true;
				if (selectedType != 0) {
...
function updateExportGridFormatter(uiObject) {
	...
	const id = $('<div />').text(gridId).html();
	const gridName = uiObject.rowData.gridName;
	const name = $('<div />').text(gridName).html();
...
function updateExportTemplateFormatter(uiObject) {
	const tempid = uiObject.rowData.templateId;
	const id = $('<div />').text(tempid).html();
	const tempname = uiObject.rowData.templateName;
	const name = $('<div />').text(tempname).html();
	let version = getVersion(uiObject);
...
function updateRBRenderer(uiObject) {
	...
	const id = $('<div />').text(resourceKey).html();
	const resourceBundleText = uiObject.rowData.resourceBundleText;
	const name = $('<div />').text(resourceBundleText).html();
	let version = getVersion(uiObject);
...
function updateExportAutocompleteFormatter(uiObject) {
	...
	const id = $('<div />').text(autocompleteId).html();
	const autocompleteDescription = uiObject.rowData.autocompleteDescription;
	const name = $('<div />').text(autocompleteDescription).html();
	let version = getVersion(uiObject);
...
function updateNotificationRenderer(uiObject) {
	...
	const id = $('<div />').text(notificationId).html();
	const messageText = uiObject.rowData.messageText;
	const name = $('<div />').text(messageText).html();
	let version = getVersion(uiObject);
...
function updateScriptLibFormatter(uiObject) {
	const scriptLibid = uiObject.rowData.script_lib_id;
	const id = $('<div />').text(scriptLibid).html();
	const libraryName = uiObject.rowData.library_name;
	const name = $('<div />').text(libraryName).html();
	let version = getVersion(uiObject);
	const isSystemVariable = 1;
	const moduleType = "ScriptLibrary";
...
function updateDynamicFormRenderer(uiObject) {
	const formId = uiObject.rowData.formId;
	const id = $('<div />').text(formId).html();
	const formName = uiObject.rowData.formName;
...
function updateFileManagerRenderer(uiObject) {
	...
	const id = $('<div />').text(fileBinId).html();
...
function updateDynaRestRenderer(uiObject) {
	const dynarestId = uiObject.rowData.dynarestId;
	const id = $('<div />').text(dynarestId).html();
	const dynarestUrl = uiObject.rowData.dynarestUrl;
...
function updateSiteLayoutRenderer(uiObject) {
	...
	const id = $('<div />').text(moduleId).html();
...
function updateAppConfigRenderer(uiObject) {
	const propertyMasterId = uiObject.rowData.propertyMasterId;
	const id = $('<div />').text(propertyMasterId).html();
	const propertyName = uiObject.rowData.propertyName;
	const name = $('<div />').text(propertyName).html();
...
function updateUsersRenderer(uiObject) {
	const user_id = uiObject.rowData.user_id;
	const id = $('<div />').text(user_id).html();
	const username = uiObject.rowData.firstName + " " + uiObject.rowData.lastName;
	const name = $('<div />').text(username).html();
	const version = "NA";
	const isSystemVariable = 2;
...
function updateRolesRenderer(uiObject) {
	...
	const id = $('<div />').text(roleName).html();
	const roleDescription = uiObject.rowData.roleDescription;
	const name = $('<div />').text(roleDescription).html();
	const version = "NA";
	const isSystemVariable = 2;
...
function formatgridId(uiObject) {
	const gridId = uiObject.rowData.gridId;
	var encodedgridId = $('<div />').text(gridId).html();
	return encodedgridId;
}
function formatgridTableName(uiObject) {
	const gridTableName = uiObject.rowData.gridTableName;
	var encodedName = $('<div />').text(gridTableName).html();
	return encodedName;
}
...
function formatautocompleteId(uiObject) {
	const autocompleteId = uiObject.rowData.autocompleteId;
	var encodedautocompleteId = $('<div />').text(autocompleteId).html();
	return encodedautocompleteId;
}
function formatfileId(uiObject) {
	const fileBinId = uiObject.rowData.fileBinId;
	var encodedfileId = $('<div />').text(fileBinId).html();
	return encodedfileId;
}
function formatformName(uiObject) {
	const formName = uiObject.rowData.formName;
	var encodedName = $('<div />').text(formName).html();
	return encodedName;
}
function formatdynarestUrl(uiObject) {
	const dynarestUrl = uiObject.rowData.dynarestUrl;
	var encodedUrl = $('<div />').text(dynarestUrl).html();
	return encodedUrl;
}
function formatmethodName(uiObject) {
	const methodName = uiObject.rowData.methodName;
	var encodedName = $('<div />').text(methodName).html();
	return encodedName;
}
...
function formatresourceBundleText(uiObject) {
	const resourceBundleText = uiObject.rowData.resourceBundleText;
	var encodedText = $('<div />').text(resourceBundleText).html();
	return encodedText;
}
function formattargetPlatform(uiObject) {
	const targetPlatform = uiObject.rowData.targetPlatform;
	var encodedText = $('<div />').text(targetPlatform).html();
	return encodedText;
}
function formatmessageType(uiObject) {
	const messageType = uiObject.rowData.messageType;
	var encodedText = $('<div />').text(messageType).html();
	return encodedText;
}
function formatpropertyName(uiObject) {
	const propertyName = uiObject.rowData.propertyName;
	var encodedText = $('<div />').text(propertyName).html();
	return encodedText;
}
function formatpropertyValue(uiObject) {
	const propertyValue = uiObject.rowData.propertyValue;
	var encodedText = $('<div />').text(propertyValue).html();
	return encodedText;
}
function formatroleName(uiObject) {
	const roleName = uiObject.rowData.roleName;
	var encodedText = $('<div />').text(roleName).html();
	return encodedText;
}
function formatemail(uiObject) {
	const email = uiObject.rowData.email;
	var encodedText = $('<div />').text(email).html();
	return encodedText;
}
function formatfirstName(uiObject) {
	const firstName = uiObject.rowData.firstName;
	var encodedText = $('<div />').text(firstName).html();
	return encodedText;
}
function formatlastName(uiObject) {
	const lastName = uiObject.rowData.lastName;
	var encodedText = $('<div />').text(lastName).html();
	return encodedText;
}
...
function getGridDetails(rowData, modType) {
	...
	switch (modType) {
		...
		case "Files":
			...
			break;
		case "ScriptLibrary":
			moduleId = rowData.script_lib_id
			name = rowData.library_name;
			version = rowData.max_version_id;
			if (version == null) {
				version = "1.0";
			}
	...
	map.forEach(function callback(value, key, map) {
		if (key != "htmlTableJSON") {
			...
			for (let exportData of exportableDataListMap1.values()) {
				...
				let modId = exportData.getModuleId();
				let id = $('<div />').text(modId).html();
				let modType = exportData.getModuleType();
				let isSystemVar = exportData.getIsSystemVariable();
				$('#' + modType + CSS.escape(id)).prop("checked", false);
				if (isSystemVar == 1) {
