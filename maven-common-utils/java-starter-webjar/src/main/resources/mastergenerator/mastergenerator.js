...
function createTable(columns) {
	...
	for (let iCounter = 0; iCounter < columns.length; ++iCounter) {
		...
		if (columns[iCounter]['_unsupported']) {
			...
			$(trElement).append("<td><input class='grid_enable' id='tenabled_" + iCounter + "' type='checkbox' disabled ></td>");
			$(trElement).append("<td><input class='grid_visible' id='thidden_" + iCounter + "' type='checkbox' disabled ></td>");
		} else {
		...
		if (columns[iCounter]['isMandatory'] == true && columns[iCounter]['_unsupported']) {
			showMessage("Few fields are required, but not supported ", "error");
			requiredNotSupportedErr = true;
		} else if (columns[iCounter]['isMandatory'] == false && columns[iCounter]['_unsupported']) {
			requiredNotSupportedWarn = true;
		}
	...
	for (let iCounter = 0; iCounter < columns.length; ++iCounter) {
		...
		if (columns[iCounter]['_unsupported']) {
			trElement = $("<tr class='details' style='border: 1px solid red;' ></tr>");
			$(trElement).append("<td><input class='form_enable' id='tfenabled_" + iCounter + "' type='checkbox' disabled ></td>");
			$(trElement).append("<td><input class='form_visible' id='tfhidden_" + iCounter + "' type='checkbox' disabled ></td>");
		} else {
			trElement = $("<tr class='details' ></tr>");
	...
	if (requiredNotSupportedWarn === true) {
		showMessage("Few fields are not supported ", "warn");
	}
...
function createMaster() {
	if (requiredNotSupportedWarn == true) {
		...
		$("#saveCreateMasterConfirm").dialog({
			bgiframe: true,
			autoOpen: true,
			modal: true,
			closeOnEscape: true,
...
function validateForm() {
	...
	let toggleCsrfVal = $("#toggleCsrf").val();
	let isCsrfChecked = $('#toggleCsrfChkBox').is(':checked') ? 1 : 0;
	let isCaptchaChecked = $('#toggleCaptchaChkBox').is(':checked') ? 1 : 0;
	let toggleCaptcha = $("#toggleCaptcha").val();
	if (isCsrfChecked && toggleCsrfVal == 1) {
		if (!isCaptchaChecked && toggleCaptcha == 0) {
			showMessage("Captcha required for enabling CSRF.", "warn");
			return false;
		}
	}
	...
	if (isValid === true) {
		if (requiredNotSupportedErr === true) {
			showMessage("Few fields are required, but not supported ", "error");
			isValid = false;
		}
	}
