...
function updateDataSource() {
	...
	if (tableAutocomplete != null) {
		...
		tableAutocomplete.options.requestParameters.datasourceId = $("#dataSource").find(":selected").val();
	}
	/** $.ajax({
		type : "POST",
		async: false,
...
function getModuleNames(entityId,moduleId){
		$.ajax({
			async: false,
			type: "GET",
			url: contextPath + apiPath + "/gmn",
			data: {
				entityId: entityId,
				moduleId: moduleId
			},
			success: function(data) {
				$.each(data, function(key, val) {
					linkModuleMultiselect.setSelectedObject(val);
					existingModuleIdList.push(val.moduleName);
				});
			}
		});
	}
	...
	function getAllModuleNames(){
			$.ajax({
				async: false,
				type: "GET",
				url: contextPath + apiPath + "/gamn",
				data: {
					
				},
				success: function(data) {
					$.each(data, function(key, val) {		
						allModuleList.push(val.moduleName);
					});
				}
			});
		}