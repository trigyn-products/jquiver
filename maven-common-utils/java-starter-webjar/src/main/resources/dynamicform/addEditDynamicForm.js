...
function onSaveButtonClick(a_actionType, isEdit) {
	...
	try {
		...
		showMessage(dynamicFormTexts["jws.excpValidatingForm"], "error");
		isInAction = false;
	...
	let successMsg = dynamicFormTexts["jws.informationSavedSuccessfully"];
	if (typeof getSuccessMessage == "function") {
		try {
			successMsg = getSuccessMessage();
		} catch (excp) {
			isInAction = false;
			showMessage(dynamicFormTexts["jws.excOccurSuccess"], "error");
	...
	let errorMsg = dynamicFormTexts["jws.errSaving"];
	if (typeof getErrorMessage == "function") {
		try {
			errorMsg = getErrorMessage();
		} catch (excp) {
			isInAction = false;
			showMessage(dynamicFormTexts["jws.exceGettingError"], "error");
