(function($) {
	...
	$.fn.fileUpload = function(options, selectedFiles) {
		...
		if (options.createFileBin !== undefined && options.createFileBin === true) {
			...
			$(this).find(".cm-uploadwrap").append($('<div class="dropzone-title"><img src="' + contextPath + '/webjars/1.0/images/multifileupload.svg"><strong class="browse">Browse </strong> or Drag and drop your files here</div>'));
					...
					$(context).dropzone({
						...
						error: function(data, errorMessage, xhr) {
							if (xhr != undefined && xhr.status == 403) {
								showMessage("You don't have enough privilege to access this module", "warn");
							} else {
								showMessage(errorMessage, "error");
							}
	...
	function deleteFileUploadTempIdCookie(fileUploadId) {
		if (getCookie("fileBinDetails") != "" && getCookie("fileBinDetails") != undefined) {
			...
			//console.log('Before = ' + fileBinJsonLocalArr.length);
